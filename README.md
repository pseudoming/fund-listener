...
- 📈 **盘中估值与前后端责任分离**：直连金融公开数据 API，实时测算交易日收益。通过后端核心 `ValuationDisplayNormalizer` 单点收拢复杂的日期状态机，无缝兼容 QDII 基金的 T-2 结算与“支付宝平滑对齐”模式，彻底杜绝前端各自为战造成的多页面状态不同步。
- 🛡️ **隐私至上**：所有数据处理均在设备本地完成，彻底杜绝资产信息外泄。
- 🛠️ **跨平台架构**：基于 Kotlin 构建核心，提供基于 Vue 3 的现代响应式 Web 界面与轻量 JVM 后端，并为后续 Android 客户端预留了完整基础。
...
- **`shared/`**：核心业务逻辑与 Ktor Server 引擎层。包含：
  - **统一估值规范器 (`ValuationDisplayNormalizer`)**：单点收拢 QDII 基金的 T-2 时差推导以及用户设定的对齐策略，统一向下游接口分发 `isSettled` 标志位及标准化收益指标。前端 0 业务推导，100% 信任服务端渲染输出。
...
本项目的所有提交必须通过最严苛的物理验证。项目根目录提供了统筹防线：

```bash
./delivery.sh
```

该脚本会自动调用 `frontend` 的 Playwright E2E 回归测试体系，包含：
1. **自动化逻辑校验**：确保核心路由、拦截器逻辑以及数据渲染准确无误。
2. **Golden Baseline 视觉快照拦截**：在 E2E 测试中深度集成 `toHaveScreenshot` 视觉回归测试，对关键页面（如仪表盘持仓列）进行像素级比对。任何 1px 的边距误差、Flex 错位或字体渲染截断都会立刻挂红并生成差异图，强行阻断交付。
