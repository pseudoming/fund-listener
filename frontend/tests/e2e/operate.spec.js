...
test('Automate UI deletion and OCR re-import', async ({ page }) => {
  ...
  console.log('查找“博时大中华亚太精选”...');
  const fundCard = page.locator('.fund-card:has-text("博时大中华")').first();
  if (await fundCard.count() > 0) {
    ...
    console.log('⚠️ 未在首页找到博时大中华，可能已被删除');
  }
